//Main длль-ки, внешние функции которые експортируются для екзешника и хука.
...
BOOL APIENTRY DllMain( HINSTANCE, DWORD fdwReason, LPVOID )
{
	...
	switch( fdwReason ) {
		...
		{
			GetCDllHookManager();//
			//при подключении выставляем себе самый большой приоритет, чтобы не тормозило
...
{
	if( nCode != HC_ACTION ) {//это сообщение обрабатывать нельзя
	...
	if( hookStruct->dwExtraInfo == MY_KEY_MESSAGE ) {//свои собственные мне обрабатывать не надо
		hookStruct->dwExtraInfo = 0;//подменяем, чтобы никто не заметил, проверенно, что всегда шлётся 0
		return CallNextHookEx( hNextGlobalHook, nCode, wParam, lParam );
	}
	...
	if( hookStruct->scanCode < 0 || 254 < hookStruct->scanCode ) {
		//иногда в европейских языках приходит VK_LCONTROL, с безумным scanCode.
		//это используется для написания в немецком языке символа [, нажатием AltGr + 8 в числах вверху
		return CallNextHookEx( hNextGlobalHook, nCode, wParam, lParam );
	}
